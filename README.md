...
The VS Code extension is listed as **Lattice Sync**. The extension ID is `HarshalDhaduk.lattice-sync`. Settings and commands use `lattice.*`; relay environment variables use `LATTICE_*`. See [upgrade notes](docs/UPGRADING.md) when moving from an earlier extension identity.
...
6. Click **Return to project** in the session header (or run **Lattice: Return to Project**). Keep the session to resume later, publish a PR, or explicitly discard its local worktree and branch. Discard requires the owner, no active agents or online teammates, and confirmation; remote branches and PRs remain on GitHub. Files changed after confirmation are preserved.

The automatic local relay runs independently of VS Code windows and uses a shared storage registry across profiles. Switching workspaces or closing the starting window no longer shuts down the relay. Reload all windows after updating from an older build. Closing VS Code keeps unfinished sessions available to resume; use **Return to project → Discard session and local branch** to delete one.
...
The host opens a project and starts a session. Joining downloads the session’s files into a managed local folder and opens it in the same VS Code window; guests do not need to clone or pull manually. Run an agent from the bottom panel. Teammates can click **Follow live edits** beneath that agent to watch the source change and its avatar move. Native editors also receive compatible changes and show participant avatars.
...
- TypeScript check and 47 automated tests: relay authorization/state/reconnects/persistence, targeted guidance, provider protocol adapters, worktree patch application, file review guards, CRDT convergence, 24 concurrent clients, identity binding, invite lifecycle, encrypted storage, parallel usage, budgets, archiving, membership-scoped memory search, native presence interpolation, no-clone workspace hydration/sync, and atomic conflict reservations.
...
- Native window navigation: same-window creation/join, source synchronization, return/discard cleanup, and relay survival after both windows exit. Run `npm run test:session-navigation` on macOS.
- Native branch workflow: automatic guest Git preparation, matching session base/branch, hydration of uncommitted live work, preserved main checkout, workspace-switch credentials and cancellation of queued overlapping prompts. Run `npm run test:branch-workflow`.
