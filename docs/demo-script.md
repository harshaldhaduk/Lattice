...
## How to Present Lattice to Judges and Investors
...
>
> Lattice is a coordination layer inside your IDE. Every agent declares its intent before writing. Every conflict is caught before it's applied. Agents negotiate with each other automatically. Humans approve what matters.
>
...
>
> [SHOW: Lattice sidebar open in VS Code for both Alice and Bob]
>
> They've both joined the same Lattice session. You can see their presence in real time — Alice is online, Bob is online. No one has registered a task yet.
>
...
>
> Lattice parses that task and creates an intent node — Alice owns `verifyToken`, `createSession`, and `auth/middleware.ts`. Everyone in the session can see this.
>
...
>
> Stop. Look at that. Before the edit is applied, Lattice catches it. Bob's agent wanted to edit `verifyToken` — but Alice's agent has already claimed it. The banner says: 'Alice's agent is modifying this function. Here's why.'
>
...
>
> That's Lattice. Intent-aware. Agent-coordinated. Built for the way teams actually build today."
...
**Terminal 1:** Start Lattice backend
```bash
...
```bash
code demo-repo
# Lattice extension auto-activates
```
...
- Alice creates a Lattice session → gets a code
...
- *"Before the edit lands, Lattice catches it. Alice already owns this function."*
...
- *"No merge conflicts. No duplicated work. That's Lattice."*
...
> Live Share is for pair programming — one driver, everyone sees the same screen. Lattice is for independent parallel work — three people doing different things, with agents, and Lattice making sure they don't step on each other.

**Q: Why not just use Git branches?**
> Branches work great for durable history. But during a 24-hour sprint with AI agents running, creating a branch for every micro-task is overhead you don't want. Lattice is the pre-Git layer — it coordinates during the build phase, then syncs cleanly to Git when you're ready.
...
3. **Solution:** "Lattice: intent-aware coordination for AI-native teams"
