...
The extension is distributed as `lattice-0.5.0.vsix`. Install it on every participant’s machine and reload VS Code. The CI workflow produces VSIX artifacts on Linux and macOS. Automatic Marketplace updates require a real publisher/repository and publishing credentials, which have not been configured.
